...
class MplCanvas(FigureCanvasQTAgg):
    ...
    def dropEvent(self, event):
        ...
        if '.mat' in files[0]:
            ...
            if matFullRes is None and mat is not None:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_UI(self):
        ...
        self.vis_strain_value_limit_checkBox.setChecked(True)
    ...
    def refresh_plot(self):
        ...
        if self.vis_strain_mat_checkBox.isChecked():
            ...
            if self.data[self.data_to_show]['mat'] is not None:
                self.matplot = self.sc.axes.pcolor(self.data[self.data_to_show]['mat'], cmap='jet', vmax = vmax, vmin = vmin)
                # self.sc.axes.imshow(self.data_to_show['mat'], cmap='jet', vmax = 0.2, vmin = -0.2)
            else:
                self.matplot = self.sc.axes.pcolor(self.data['18']['mat'], cmap='jet', vmax = vmax, vmin = vmin)
                # self.sc.axes.imshow(self.data['18']['mat'], cmap='jet', vmax = 0.2, vmin = -0.2)
        ...
        if self.data[self.data_to_show]['TOS'] is not None and self.vis_tos_loaded_checkBox.isChecked():
            ...
            self.sc.axes.legend([self.tos_curve_line, self.tos_loaded_line], ['new', 'loaded'])
